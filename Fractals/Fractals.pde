...
void setup() {
  ...
  Piece side_3 = new Piece(c, a, 0);
  Piece side_4 = new Piece(d, e, 0);
  Piece side_5 = new Piece(e, f, 0);
  Piece side_6 = new Piece(f, d, 0);
...
void join (Piece[] arr, ArrayList<Piece> list) {
  for (Piece side : arr) {
    counter++;
    if (dist(side.vecA.x, side.vecA.y, side.vecB.x, side.vecB.y) > 3 &&
        side.vecA.x > 40 && side.vecA.y > 10 && side.vecB.x > 40 && side.vecB.y > 10 &&
        side.vecA.x < 880 && side.vecA.y < 540 && side.vecB.x <880 && side.vecB.y < 540)
    {
      list.add(side);
      if (counter % 5 == 0)
      {
